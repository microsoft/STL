...
float __CLRCALL_PURE_OR_CDECL _XLgamma(float x) { // moderately accurate log gamma
    static constexpr float coeff[6] = {76.18009172947146F, -86.50532032941677F, 24.01409824083091F, -1.23173972450155F,
    ...
    for (unsigned int i = 0; i < 6; ++i, y += 1.0) {
        val1 += coeff[i] / y;
    }
...
double __CLRCALL_PURE_OR_CDECL _XLgamma(double x) { // moderately accurate log gamma
    static constexpr double coeff[6] = {76.18009172947146, -86.50532032941677, 24.01409824083091, -1.23173972450155,
...
long double __CLRCALL_PURE_OR_CDECL _XLgamma(long double x) { // moderately accurate log gamma
    static constexpr long double coeff[6] = {76.18009172947146, -86.50532032941677, 24.01409824083091,
        -1.23173972450155, 0.1208650973866179E-2, -0.5395239384953E-5};
