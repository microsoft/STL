...
namespace {
    ...
    [[nodiscard]] _Unsigned128 u128_from_mp(_MP_arr _Wx) noexcept {
    ...
    void assign_mp_from_u128(_MP_arr _Wx, const _Unsigned128 _Result) noexcept {
...
_CRTIMP2_PURE void __CLRCALL_PURE_OR_CDECL _MP_Add(_MP_arr _Wx, const uint64_t _Cx) noexcept {
    ...
    const auto _Result = u128_from_mp(_Wx) + _Cx;
    assign_mp_from_u128(_Wx, _Result);
}
...
_CRTIMP2_PURE void __CLRCALL_PURE_OR_CDECL _MP_Rem(_MP_arr _Wx, const uint64_t _Mx) noexcept {
    // perform "_Wx %= _Mx"
    const auto _Result = u128_from_mp(_Wx) % _Mx;
    assign_mp_from_u128(_Wx, _Result);
}
_STD_END
