...
struct alignas(16) _Base128 {
    ...
    _NODISCARD static constexpr uint64_t _UMul128(
        ...
        // vvvvvvvvvv DERIVED FROM https://github.com/ulfjack/ryu/blob/59661c3/ryu/d2s_intrinsics.h vvvvvvvvvv
        ...
        // The __doubleUnderscoreLowercase variable names are being used to avoid diverging from external code.
        const uint32_t __aLo = static_cast<uint32_t>(_Left);
        const uint32_t __aHi = static_cast<uint32_t>(_Left >> 32);
        const uint32_t __bLo = static_cast<uint32_t>(_Right);
        const uint32_t __bHi = static_cast<uint32_t>(_Right >> 32);
        ...
        _High_result = __pHi;
        return __pLo;
        // ^^^^^^^^^^ DERIVED FROM https://github.com/ulfjack/ryu/blob/59661c3/ryu/d2s_intrinsics.h ^^^^^^^^^^
    }
