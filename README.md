...
`C:\Dev\STL`, build outputs will end up at `C:\Dev\STL\out\build\x64\out`. Add the `inc` directory to your include
paths, and the `lib\{architecture}` directory to your library path. The names of the import and static libraries have
the same names as those that ship with MSVC, meaning the existing compiler `/MD`, `/MDd`, `/MT`, or `/MTd` switches will
work without modification of your build scripts or command line muscle memory.
...
A complete example using the DLL versions from an "x64 Native Tools Command Prompt for VS 2019" follows. The compiler
looks for include directories according to the `INCLUDE` environment variable, and the linker looks for import libraries
according to the `LIB` environment variable. The Windows loader will look for DLL dependencies in `PATH`.
