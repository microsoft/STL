...
you need to hook up to your build system to use the freshly built STL. Assuming you built the x64 target with the Visual
Studio IDE, with the STL repository cloned to `C:\Dev\STL`, build outputs will end up at `C:\Dev\STL\out\build\x64\out`.
Add the `inc` directory to your include paths, and the `lib\{architecture}` directory to your library path. The names
of the import and static libraries have the same names as those that ship with MSVC, meaning the existing compiler
`/MD`, `/MDd`, `/MT`, or `/MTd` switches will work without modification of your build scripts.

If you choose to use the DLL versions of the libraries, note that the DLLs generated by the CMake build system here have
a suffix, defaulting to `_oss`, which distinguishes them from the binaries that ship with MSVC. This avoids any
potential conflicts with the DLLs installed by the [redistributables][Redist] into System32, and ensures other system
components that want to be a "guest in your process" like print drivers and shell extensions can still see the version
of the STL and export surface they were built with. The DLLs to deploy with your application end up in
`bin\{architecture}`.

For example, to get a command prompt where the `cl` there will link with your freshly built STL:

1. Open an "x64 Native Tools Command Prompt for VS 2019".
2. Prepend the `inc` here to `INCLUDE`: `set INCLUDE=C:\Dev\STL\out\build\x64\out\inc;%INCLUDE%`
3. Prepend the appropriate `lib` here to `LIB`: `set LIB=C:\Dev\STL\out\build\x64\out\lib\amd64;%LIB%`
4. If using the DLL versions, prepend the appropriate `bin` to PATH:
   `set PATH=C:\Dev\STL\out\build\x64\out\bin\amd64;%PATH%`
5. Compile from the command line as normal.

A complete example from an x64 Native Tools Command Prompt for VS 2019, showing that the "OSS" import libraries used:
...
[Redist]: https://support.microsoft.com/en-us/help/2977003/the-latest-supported-visual-c-downloads
