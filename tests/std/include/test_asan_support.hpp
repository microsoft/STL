...
extern "C" void* __sanitizer_contiguous_container_find_bad_address(
    const void* beg, const void* mid, const void* end) noexcept;
    ...
    namespace asan {
        ...
        inline void print_shadow_bytes(const void* addr, size_t num_bytes, const void* error_addr = nullptr,
            ...
            const char* begin = reinterpret_cast<const char*>(
                reinterpret_cast<uintptr_t>(addr) & ~(shadow_granularity - 1)); // align down to shadow boundary
            ...
            fprintf(stderr, "Shadow for addresses %p -> %p (%p -> %p):", begin, end, shadow_addr_of(begin),
                shadow_addr_of(end));

            const char* const print_begin = reinterpret_cast<const char*>(
                reinterpret_cast<uintptr_t>(begin) - bytes_per_line_mask & ~bytes_per_line_mask);
            const char* const print_end = reinterpret_cast<const char*>(
                (reinterpret_cast<uintptr_t>(end) + bytes_per_line_mask) & ~bytes_per_line_mask);
            for (const char* p = print_begin; p < print_end; p += 8) {
                ...
                if (reinterpret_cast<uintptr_t>(p)
                    == (reinterpret_cast<uintptr_t>(error_addr) & ~(shadow_granularity - 1))) {
        ...
        bool verify_container_poisoning(
            const void* const addr, const size_t valid_size, const size_t total_capacity, const bool is_overpoisoned) {
            ...
            if (bad_addr) {
                ...
                const unsigned char expected_byte =
                    static_cast<unsigned char>((bad_addr >= b1 && bad_addr < b2) ? static_cast<unsigned char>(mid - b1)
                                               : (bad_addr < mid)                ? 0
                                                                                 : 0xfc);
