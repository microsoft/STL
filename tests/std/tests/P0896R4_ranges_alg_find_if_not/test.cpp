...
constexpr void smoke_test() {
    ...
    std::array<P, 3> data = {{{0, 13}, {0, 13}, {0, 13}}};
    ...
    for (auto& [value, _] : data) {
        value = 42;
        {
            ...
            auto result = find_if_not(move_only_range{data}, equals(0), get_first);
        ...
        }
        {
            ...
            auto result = find_if_not(x.begin(), x.end(), equals(0), get_first);
            STATIC_ASSERT(same_as<decltype(result), iterator_t<move_only_range<P>>>);
            assert((*result).first == 42);
        }
        value = 0;
