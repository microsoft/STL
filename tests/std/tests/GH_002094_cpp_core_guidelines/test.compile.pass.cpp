...
#pragma warning(default : 26483) // Value %1$lld is outside the bounds (0, %2$lld) of variable '%3$s'.
                                 // Only index into arrays using constant expressions that are within bounds of the
                                 // array (bounds.2).
#pragma warning(default : 26486) // Don't pass a pointer that may be invalid to a function. Parameter %1$d '%2$s' in
                                 // call to '%3$s' may be invalid (lifetime.3).
#pragma warning(default : 26487) // Don't return a pointer '%1$s' that may be invalid (lifetime.4).
#pragma warning(default : 26488) // Do not dereference a potentially null pointer: '%1$s'. '%2$s' was null at line %3$u
                                 // (lifetime.1).
#pragma warning(default : 26489) // Don't dereference a pointer that may be invalid: '%1$s'. '%2$s' may have been
                                 // invalidated at line %3$u (lifetime.1).
#pragma warning(default : 26814) // The const variable '%1$s' can be computed at compile-time.
                                 // Consider using constexpr (con.5).
#pragma warning(default : 26815) // The pointer is dangling because it points
                                 // at a temporary instance which was destroyed.
#pragma warning(default : 26816) // The pointer points to memory allocated on the stack.
#pragma warning(default : 26817) // Potentially expensive copy of variable '%1$s' in range-for loop. Consider making it
                                 // a const reference (es.71).
