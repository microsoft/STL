...
#if _HAS_CXX17
using validating_hash = tagged_hash<holder<incomplete>>;
#endif // _HAS_CXX17
...
void test_algorithms() {
...
#if _HAS_CXX17
    (void) std::search(varr, varr + 1, std::default_searcher<const validator*>{varr, varr + 1});
    (void) std::search(iarr, iarr + 1, std::default_searcher<const int*, validating_equal>{iarr, iarr + 1});
    // WIP: dependence on iterator comparison
    // (void) std::search(varr, varr + 1, std::boyer_moore_searcher<const validator*>{varr, varr + 1});
    (void) std::search(iarr, iarr + 1, std::boyer_moore_searcher<const int*, validating_hash>{iarr, iarr + 1});
    // WIP: dependence on iterator comparison
    // (void) std::search(varr, varr + 1, std::boyer_moore_horspool_searcher<const validator*>{varr, varr + 1});
    (void) std::search(iarr, iarr + 1, std::boyer_moore_horspool_searcher<const int*, validating_hash>{iarr, iarr + 1});
#endif // _HAS_CXX17
    ...
    // (void) std::partition(iarr, iarr, validating_zero_equality{}); // requires Cpp17ValueSwappable

    // (void) std::stable_partition(varr, varr, simple_zero_equality{}); // requires Cpp17ValueSwappable
    // (void) std::stable_partition(iarr, iarr, validating_zero_equality{}); // requires Cpp17ValueSwappable
...
void test_per_execution_policy() {
    ...
    // (void) std::partition(ExecutionPolicy, iarr, iarr, validating_zero_equality{}); // requires Cpp17ValueSwappable

    // (void) std::stable_partition(
    //    ExecutionPolicy, varr, varr, simple_zero_equality{}); // requires Cpp17ValueSwappable
    // (void) std::stable_partition(
    //    ExecutionPolicy, iarr, iarr, validating_zero_equality{}); // requires Cpp17ValueSwappable
...
// Also test GH-3842
void test_gh_3842() {
    std::vector<std::shared_ptr<holder<incomplete>>> vec(5);
    (void) vec;
}
