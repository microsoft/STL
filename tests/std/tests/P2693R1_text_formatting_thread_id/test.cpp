...
struct FormatFn {
    ...
    [[nodiscard]] auto operator()(
        const basic_format_string<CharT, type_identity_t<Args>...> str, Args&&... args) const {
...
struct VFormatFn {
    template <class... Args>
    [[nodiscard]] auto operator()(const basic_string_view<CharT> str, Args&&... args) const {
        if constexpr (same_as<CharT, char>) {
            return vformat(str, make_format_args(forward<Args>(args)...));
        } else {
            return vformat(str, make_wformat_args(forward<Args>(args)...));
        }
    }
};
...
public:
    template <class... Args>
    auto operator()(const basic_string_view<CharT> str, Args&&... args) const {
        try {
            (void) base(str, forward<Args>(args)...);
...
private:
    struct StringInserter {
        ...
        using container_type    = basic_string<CharT>;
...
public:
    static_assert(output_iterator<StringInserter, CharT>);
    static_assert(!copyable<StringInserter>);
    static_assert(movable<StringInserter>);

    template <class... Args>
    [[nodiscard]] auto operator()(
        const basic_format_string<CharT, type_identity_t<Args>...> str, Args&&... args) const {
        return format_to(StringInserter{}, str, forward<Args>(args)...).str;
    }
};
