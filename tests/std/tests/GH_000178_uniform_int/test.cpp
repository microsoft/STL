...
bool basic_test() {
    constexpr auto max         = (1ull << 60);
    constexpr auto num_bins    = 20ull; // Don't change this without looking up a new threshold below.
    constexpr auto bin_width   = max / num_bins; // except possibly the last bin
    constexpr auto bin_freq    = static_cast<double>(bin_width) / max;
    constexpr auto freq_rem    = static_cast<double>(max % bin_width) / max;
    constexpr double threshold = 31.410; // chi-squared critical value for d.f. = 20 and p = 0.05

    Generator gen;
    std::uniform_int_distribution<uint64_t> dist(0, max - 1u);


    const int N             = 20'000;
    int frequency[num_bins] = {0};
    for (int i = 0; i < N; ++i) {
        ++frequency[std::min(dist(gen) / bin_width, num_bins - 1u)];
    }
...
bool test_modulus_bias() {
    ...
    constexpr int max          = 5; // Don't change this without looking up a new threshold below.
    constexpr double threshold = 11.07; // chi-squared critical value for d.f. = 5 and p = 0.05
    std::uniform_int_distribution<> rng(0, max - 1);
    std::independent_bits_engine<std::mt19937, 3, uint32_t> gen;
    ...
    double chi_squared = 0.0;
    for (int i = 0; i < max; ++i) {
        const double expected = static_cast<double>(N) / max;
...
int main() {
    ...
    // (4) URBG is called multiple times and overflow the number of bits in the type
    assert((basic_test<mt19937_64>()));
    assert((basic_test<independent_bits_engine<std::mt19937_64, 61, uint64_t>>()));
    assert((basic_test<independent_bits_engine<std::mt19937, 31, uint32_t>>()));
    assert((basic_test<independent_bits_engine<std::mt19937, 25, uint32_t>>()));
