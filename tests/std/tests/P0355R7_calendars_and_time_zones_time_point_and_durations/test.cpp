...
#define DURATION_TEST(TYPE, BITS, ...)                        \
    static_assert(is_integral_v<TYPE::rep>);                  \
    static_assert(is_signed_v<TYPE::rep>);                    \
    static_assert(numeric_limits<TYPE::rep>::digits >= BITS); \
    static_assert(is_same_v<TYPE::period, __VA_ARGS__>);
