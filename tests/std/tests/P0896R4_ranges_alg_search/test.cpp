...
struct instantiator {
    ...
    static constexpr void call() {
        ...
        {
            auto result =
                ranges::search(ranges::begin(range2), ranges::end(range2), ranges::begin(range2), ranges::end(range2));
        ...
        {
            auto result = ranges::search(range1, range2, ranges::equal_to{}, get_first);
            STATIC_ASSERT(same_as<decltype(result), ranges::subrange<ranges::iterator_t<Fwd1>>>);
        ...
        constexpr auto plus1 = [](int x) { return x + 1; };
        {
            auto result = ranges::search(range1, range2, ranges::equal_to{}, get_first, plus1);
            STATIC_ASSERT(same_as<decltype(result), ranges::subrange<ranges::iterator_t<Fwd1>>>);
        ...
        {
            auto result = ranges::search(ranges::begin(range1), ranges::end(range1), ranges::begin(range2),
                ranges::end(range2), ranges::equal_to{}, get_first, plus1);
            STATIC_ASSERT(same_as<decltype(result), ranges::subrange<ranges::iterator_t<Fwd1>>>);
            assert(result.begin() == ranges::next(range1.begin(), 3));
            assert(result.end() == ranges::next(range1.begin(), 6));
        }
        ...
        // negative case
        {
            auto result = ranges::search(range2, range1, ranges::equal_to{}, plus1, get_first);
            STATIC_ASSERT(same_as<decltype(result), ranges::subrange<ranges::iterator_t<Fwd2>>>);
            assert(result.empty());
        }
        {
            auto result = ranges::search(ranges::begin(range2), ranges::end(range2), ranges::begin(range1),
                ranges::end(range1), ranges::equal_to{}, plus1, get_first);
            STATIC_ASSERT(same_as<decltype(result), ranges::subrange<ranges::iterator_t<Fwd2>>>);
            assert(result.empty());
        }
    }
