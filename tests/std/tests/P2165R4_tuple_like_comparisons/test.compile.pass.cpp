...
constexpr bool test() {
    ...
    { // Check comparisons of (tuple_size_v == 2) tuple-like_types
        ...
        static_assert(tuple{1, 2} == array<int, 2>{1, 2} && tuple<int, int>{2, 1} != array<int, 2>{1, 2});
        static_assert(is_eq(tuple{1, 2} <=> array{1, 2}) && is_lt(tuple{1, 1} <=> array{1, 2})
                      && is_gt(tuple{2, 1} <=> array{1, 1}));

        static_assert(tuple{1, 2} == pair{1, 2} && tuple{2, 1} != pair{1, 2});
        static_assert(is_eq(tuple{1, 2} <=> pair{1, 2}) && is_lt(tuple{1, 1} <=> pair{1, 2})
                      && is_gt(tuple{2, 1} <=> pair{1, 1}));

        int a    = 0;
        int b    = 1;
        int c[2] = {2, 3};
        static_assert(tuple{&a, &b} == subrange{&a, &b} && tuple{&b, &a} != subrange{&a, &b});
        static_assert(is_eq(tuple{&a, &b} <=> pair{&a, &b}) && is_lt(tuple{&c[0], &c[0]} <=> pair{&c[0], &c[1]})
                      && is_gt(tuple{&c[1], &c[0]} <=> pair{&c[0], &c[1]}));
    }
    ...
    { // Check comparisons of (tuple_size_v == 3) tuple-like types
        ...
        static_assert(tuple{1, 2, 3} == array{1, 2, 3} && tuple{6, 7, 8} != array{7, 8, 9});
        static_assert(is_eq(tuple{1, 2, 3} <=> array{1, 2, 3}) && is_lt(tuple{1, 2, 3} <=> array{1, 2, 4})
                      && is_gt(tuple{4, 3, 2} <=> array{3, 2, 1}));
    }
