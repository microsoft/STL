...
[[noreturn]] void throw_get_last_error(const char *routine) {
    ...
    FormatMessageA(
        ...
        error_buffer,
        static_cast<DWORD>(std::size(error_buffer)),
...
struct service_handle {
    ...
    service_handle(SC_HANDLE scm, const char *name) noexcept
        : h(OpenService(scm, name, SERVICE_ALL_ACCESS))
    {}
...
driver_loader::driver_loader(const char *path, const char *name)
    : driver_name(name)
{
